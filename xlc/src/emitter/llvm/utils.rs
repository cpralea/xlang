...
impl<'a> emitter::Emitter<'a> {
    pub fn find_str_decl_by_ir_id(
        str_decls: &'a Vec<strdecl::StrDecl<'a>>,
        ir_id: &String,
    ) -> Option<&'a strdecl::StrDecl<'a>> {
        str_decls.iter().find(|e| e.ir_id.as_str() == ir_id)
    }

    pub fn find_str_decl_by_xl_val(
        str_decls: &'a Vec<strdecl::StrDecl<'a>>,
        xl_val: &String,
    ) -> Option<&'a strdecl::StrDecl<'a>> {
    ...
    pub fn find_var_decl_by_xl_id(
        var_decls: &'a Vec<vardecl::VarDecl<'a>>,
        xl_id: &String,
    ) -> Option<&'a vardecl::VarDecl<'a>> {
        var_decls.iter().find(|e| e.xl_id == xl_id)
    }

    pub fn get_str_decl_by_ir_id(
        str_decls: &'a Vec<strdecl::StrDecl<'a>>,
        ir_id: &String,
    ) -> &'a strdecl::StrDecl<'a> {
    ...
    pub fn ir(ir: &mut String, level: usize, line: &str) {
        ir.push_str(
            format!("{}{}{}", common::take(level, common::TAB), line, common::NL).as_str(),
        );
    }
}
