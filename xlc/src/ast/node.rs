...
pub struct Node<'a> {
    ...
    pub token: Option<&'a token::Token>,
}


impl<'a> Node<'a> {
    pub fn new(kind: Box<NodeKind<'a>>, token: Option<&'a token::Token>) -> Node<'a> {
        Node {
            kind: kind,
            token: token,
        }
...
pub enum NodeKind<'a> {
    ...
    Print { expression: Option<Node<'a>> },
    Program { statements: Nodes<'a> },
    Unknown,
}
...
pub trait NodeVisitor<'a> {
    ...
    fn should_descend_into_node(&mut self, _node: &'a Node<'a>) -> bool {
        true
    }
    ...
    fn should_descend_into_assignment(&mut self, _assignment: &'a Node<'a>) -> bool {
        true
    }
    ...
    fn should_descend_into_expression(&mut self, _expression: &'a Node<'a>) -> bool {
        true
    }
    ...
    fn should_descend_into_print(&mut self, _print: &'a Node<'a>) -> bool {
        true
    }
    ...
    fn should_descend_into_program(&mut self, _program: &'a Node<'a>) -> bool {
        true
    }
    ...
    fn should_descend_into_unknown(&mut self, _program: &'a Node<'a>) -> bool {
        true
    }
    ...
    fn descend_into_expression<T>(&'a self, visitor: &mut NodeVisitor<'a, Result = T>) {
        ...
        match *self.kind {
            NodeKind::Expression {
                ...
                match *left {
                    Some(ref left) if descend => left.accept(visitor),
                    _ => {}
                }
                match *right {
                    Some(ref right) if descend => right.accept(visitor),
                    _ => {}
                }
            }
            _ => unreachable!(),
