...
pub fn parse_cmd_line() -> common::Status<Configuration> {
        ...
        (false, false, None::<String>, String::new());
    {
        ...
        parser
            .refer(&mut verbose)
            .add_option(&["-v", "--verbose"], argparse::StoreTrue, "verbose compiler output");
        parser
            .refer(&mut no_output)
            .add_option(&["--no-output"],
                        argparse::StoreTrue,
    ...
    config
        .opts
        .insert(String::from(OPT_OUTPUT), match output {
            Some(output) => output,
            None => {
                format!("{}.ll", match file.ends_with(".xl") {
